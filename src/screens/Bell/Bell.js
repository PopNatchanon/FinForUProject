    ...
    let dataNewStore = (
        Deal_Store.map((value, index) => {
            ...
            return <TouchableOpacity activeOpacity={1} key={index} onPress={() =>
                NavigationNavigate({ goScreen: 'Store', setData: { id_item: item.id_store }, navigation })}>
    ...
    let for_U_Item = (
        for_U_Store.map((value, index) => {
            return <TouchableOpacity key={index} activeOpacity={1} onPress={() =>
                NavigationNavigate({ goScreen: 'Bell_Detail_Promotion', navigation })}>
...
export let Update_buy = (props) => {
    ...
    let Updatebuy = (
        Updateitem.map((value, index) => {
            return <TouchableOpacity key={index} activeOpacity={1} onPress={() =>
                NavigationNavigate({ goScreen: 'Bell_Detail_Order', navigation })}>
