...
export let Button_bar = (props) => {
    ...
    useEffect(() => {
        !activeGetCurrentUser && activeSelectedIndex && cokie && GetServices({ uriPointer: uri, Authorization: cokie, showConsole: 'view_purchase', dataBody, getDataSource: value => getData(value), });
    ...
    let PathList = <View>
        {!activeSelectedIndex && ([
            <Text key={'wait'} style={[stylesFont.FontFamilyText, stylesFont.FontSize5, { marginLeft: 10, marginTop: 10, }]}>{selectedIndex == 1 && ''}</Text>,
        ...
        <View style={stylesMain.FrameBackground}>
            <View style={[stylesProfileTopic.Order_BoxStore]}>
                <View style={stylesMain.FlexRow}>
                    <FastImage style={[stylesProfileTopic.Order_StorePro, stylesMain.ItemCenterVertical,]} source={{ uri: uri_image_Customer, }}
                        resizeMode={FastImage.resizeMode.contain} />
                    <Text style={[stylesFont.FontFamilyBold, stylesFont.FontSize5, stylesMain.ItemCenterVertical, { marginLeft: 5 }]}>{dataService.customer_name}</Text>
                </View>
                {[
                    ...
                    dataService.purchase == 'paid_unsend' && <Text key={'shipping_order'} style={[stylesFont.FontFamilyText, stylesFont.FontSize6, stylesMain.ItemCenterVertical, { color: '#111', width: width * 0.3, textAlign: 'center', }]}>เพิ่มเลขพัสดุ</Text>,
                    dataService.purchase == 'paid' && <Text key={'shipping'} style={[stylesFont.FontFamilyText, stylesFont.FontSize6, stylesMain.ItemCenterVertical]}>{dataService.tracking_number}</Text>,
                    dataService.purchase == 'accepted' && <TouchableOpacity key={'Review'} activeOpacity={1}>
                        <Text style={[stylesFont.FontFamilyText, stylesFont.FontSize6, stylesMain.ItemCenterVertical, { color: '#20BDA1', width: width * 0.3, textAlign: 'center', }]}><IconFeather name='edit' size={15} />รอการรีวิว</Text>
                    </TouchableOpacity>,
                    dataService.purchase == 'reviewed' && <TouchableOpacity key={'Review_order'} activeOpacity={1} onPress={() =>
                        ...
                        <Text style={[stylesFont.FontFamilyText, stylesFont.FontSize6, stylesMain.ItemCenterVertical, { color: '#111', width: width * 0.3, textAlign: 'center', }]}>สำเร็จแล้ว</Text>
                    </TouchableOpacity>]}
            </View>
            <View style={stylesProfileTopic.Order_Product}>
                <View style={stylesMain.FlexRow}>
                    <View style={stylesProfileTopic.Order_Product_Pro}>
                        <FastImage style={stylesMain.BoxProduct1Image} source={{ uri: uri_image_product, }} resizeMode={FastImage.resizeMode.contain} />
                    </View>
                    <View style={{ marginTop: 10, width: width * 0.5 }}>
                        ...
                        <Text numberOfLines={2} style={[stylesFont.FontFamilyText, stylesFont.FontSize7,]}>{`${dataService.detail_1} ${dataService.detail_2}`}</Text>
                ...
                <NumberFormat value={dataService.price} displayType={'text'} thousandSeparator={true} prefix={'฿'} renderText={value =>
                    <Text style={[stylesFont.FontFamilyText, stylesFont.FontSize5, { color: mainColor, marginTop: 10, }]}>{value}</Text>} />            </View>
            <View style={stylesProfileTopic.Order_Box_price}>
                ...
                <View style={[stylesProfileTopic.Order_Box_priceText, { marginTop: 5, }]}>
                    <TouchableOpacity onPress={() => NavigationNavigateScreen({ goScreen: 'Seller_Detail_Order', setData: { selectedIndex: 0 }, navigation })}>
