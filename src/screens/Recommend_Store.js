...
function Recommend_Store(props) {
    ...
    useEffect(() => { activeGetSource && GetData({ getCokie: true, getUser: true, getSource: value => { setActiveGetSource(false); setCokie(value.keycokie); setCurrentUser(value.currentUser); } }); }, [activeGetSource]);
    useEffect(() => { activeGetSource == false && activeGetServices && id_slide && cokie && GetServices({ key: name_path, uriPointer: uri, dataBody, Authorization: cokie, getDataSource: value => { setActiveGetServices(false); setDataService(value); } }); }, [activeGetSource == false && activeGetServices && id_slide && cokie]);
    return (
        <SafeAreaView style={stylesMain.SafeAreaView}>
            ...
            <ScrollView>
                <Header dataService={{ slide_image: dataService && dataService.slide_image, list_slide: dataService && dataService.list_slide }} />
        ...
        return dataService && dataService.list_slide && dataService.list_slide.length > 0 ?
            (dataService.list_slide.map((value, index) => {
                ...
                return (<View key={index} style={stylesTopic.Header} >
                    <FastImage source={{ uri: image_header, }} style={stylesTopic.Header_ImageBackground} resizeMode={FastImage.resizeMode.stretch} />
                    <Text style={[stylesTopic.Header_Text, stylesFont.FontFamilyBold]}></Text>
                    <View style={stylesTopic.Header_BoxDetail}>
                        <Text style={[stylesFont.FontFamilyBold, stylesFont.FontSize4, stylesTopic.Header_DetailText, { textAlign: 'center', }]}>{value.header}</Text>
...
export class Store_Detail extends React.Component {
    ...
    starReview(star, starSize) {
        let starBox = [];
        for (var n = 0; n < 5; n++) {
            if (star > n) { starBox.push(<IconFontAwesome style={stylesDetail.Price_IconStar} key={n} name='star' size={starSize ?? 20} color='#FFAC33' />) }
            else { starBox.push(<IconFontAwesome style={stylesDetail.Price_IconStar} key={n} name='star' size={starSize ?? 20} color='#E9E9E9' />); };
        };
        return starBox;
    };
    setStateFollow = () => {
    ...
    render() {
        ...
        activeGetServices && cokie && GetServices({ uriPointer: uri, dataBody, Authorization: cokie, getDataSource: this.getData.bind(this), });
        return (
            <View style={{ backgroundColor: '#FFFFFF', width: '100%', marginVertical: 5 }}>
                <View>
                    ...
                    <View style={stylesTopic.Store_Box}>
                        <TouchableOpacity onPress={() => NavigationNavigateScreen({ goScreen: 'StoreScreen', setData: { id_item: dataService.id_store }, navigation })}>
                            <View style={[stylesTopic.Store_Pro, stylesMain.ItemCenter]}>
                                <FastImage style={{ height: '70%', width: '70%' }} source={{ uri: image_store }} resizeMode={FastImage.resizeMode.contain} />
                            </View>
                        </TouchableOpacity>
                        <View style={{ margin: 10, }}>
                            <TouchableOpacity onPress={() => NavigationNavigateScreen({ goScreen: 'StoreScreen', setData: { id_item: dataService.id_store }, navigation })}>
                                <View style={stylesTopic.Store_Name}>
                                    <Text style={[stylesTopic.Store_NameText, stylesFont.FontFamilyBold, stylesFont.FontSize6]}>{dataService.store_name}</Text>
                                </View>
                            </TouchableOpacity>
                                ...
                                <View style={stylesTopic.Store_BoxButton}>
                                    <View style={stylesTopic.Store_Button}>
                                        <TouchableOpacity onPress={() => this.setStateFollow()}>
                                            <Text style={[stylesFont.FontFamilyText, { textAlign: 'center', color: '#0A55A6' }]}>{dataService2 && dataService2.output}</Text>
                                        </TouchableOpacity>
                                    </View>
                                    <TouchableOpacity onPress={() => NavigationNavigateScreen({ goScreen: 'StoreScreen', setData: { id_item: dataService.id_store }, navigation })} style={[stylesTopic.Store_Button, { backgroundColor: '#0A55A6', marginLeft: 8, }]}>
                    ...
                    <View>
                        ...
                        <View style={stylesTopic.Store_Product}>
                            {dataService && dataService.product && <FlatProduct {...this.props} custumNavigation='Store_Detail'
                                dataService={dataService.product} mode='row3' nameFlatProduct='Store_Detail' nameSize={14} priceSize={15}
                                dispriceSize={15} />}
                        </View>
                    </View>
                </View>
