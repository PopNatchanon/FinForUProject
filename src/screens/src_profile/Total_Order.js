...
export let Button_bar = (props) => {
    ...
    let updateIndex = (value) => {
        ...
        setLoading(true);
        setSelectedIndex(value.selectedIndex * 1);
    };
    let PathList = () => {
        switch (selectedIndex) {
            case 0:
                activeSelectedIndex == true && currentUser && cokie && selectedIndex == 0 &&
                    GetServices({
                    ...
                    <>
                        {
                            activeSelectedIndex == false && selectedIndex == 0 && ([
                                ...
                                { marginLeft: 10, marginTop: 10, }]}> รายการคำสั่งซื้อ</Text>,
                                dataService && dataService.purchase.length > 0 ? dataService.purchase.map((value, index) => {
            ...
            case 1:
                activeSelectedIndex == true && currentUser && cokie && selectedIndex == 1 &&
                    GetServices({
                        uriPointer: uri, Authorization: cokie, showConsole: 'view_purchase', dataBody, getDataSource: value => getData(value),
                    })
                return (
            ...
            case 2:
                activeSelectedIndex == true && currentUser && cokie && selectedIndex == 2 &&
                    GetServices({
                        uriPointer: uri, Authorization: cokie, showConsole: 'view_purchase', dataBody, getDataSource: value => getData(value),
                    })
                return (
                    <>
                        {
                            activeSelectedIndex == false && selectedIndex == 2 && ([
                                ...
                                { marginLeft: 10, marginTop: 10, }]}>ที่ต้องได้รับ</Text>,
                                dataService && dataService.purchase.length > 0 ? dataService.purchase.map((value, index) => {
                                    return <From_Order_Box {...props} dataService={value} key={index} />
                                }) : <View style={[stylesProfileTopic.products_pro, { height: height * 0.5 }]}>
                                        <IconFeather name='edit' size={50} color='#000000' />
            ...
            case 3:
                activeSelectedIndex == true && currentUser && cokie && selectedIndex == 3 &&
                    GetServices({
                        uriPointer: uri, Authorization: cokie, showConsole: 'view_purchase', dataBody, getDataSource: value => getData(value),
                    })
                return (
            ...
            case 4:
                activeSelectedIndex == true && currentUser && cokie && selectedIndex == 4 &&
                    GetServices({
                        uriPointer: uri, Authorization: cokie, showConsole: 'view_purchase', dataBody, getDataSource: value => getData(value),
                    })
                return (
                    <>
                        {
                            activeSelectedIndex == false && selectedIndex == 4 && ([
                                ...
                                { marginLeft: 10, marginTop: 10, }]}>ยกเลิกสินค้า</Text>,
                                dataService && dataService.purchase.length > 0 ? dataService.purchase.map((value, index) => {
                                    return <From_Order_Box {...props} dataService={value} key={index} />
                                }) : <View style={[stylesProfileTopic.products_pro, { height: height * 0.5 }]}>
                                        <IconFeather name='edit' size={50} color='#000000' />
        ...
        <>
            <View style={stylesProfileTopic.Button_bar}>
                <ScrollView horizontal>
                    <TabBar sendData={value => updateIndex(value)} item={item} SetValue={selectedIndex >= 0 ? selectedIndex :
                        setFSelectedIndex} activeColor={'#fff'} activeFontColor={mainColor} type='tag' />
                </ScrollView>
            </View>
            <ScrollView>
                {PathList()}
            ...
            <View style={stylesMain.FrameBackground}>
                <View style={[stylesProfileTopic.Order_BoxStore]}>
                    <View style={stylesMain.FlexRow}>
                        <FastImage style={[stylesProfileTopic.Order_StorePro, stylesMain.ItemCenterVertical,]} source={
                            { uri: uri_image_store, }} resizeMode={FastImage.resizeMode.contain} />
                        ...
                        <View style={stylesProfileTopic.Order_Box_Button}>
                            <TouchableOpacity>
                                ...
                                { borderWidth: 1, borderColor: mainColor, backgroundColor: mainColor }]}>
                                    ...
                                    <Text style={[stylesFont.FontFamilyText, stylesFont.FontSize6, { color: '#FFFFFF', marginHorizontal: 6, }]
                                    }>แชทเลย</Text>
                                </View>
                            </TouchableOpacity>
                            <TouchableOpacity>
                    ...
                    {[
                        ...
                        dataService.status_purchase == 'paid' && (
                            dataService.tracking_number == null ? <Text key={'shipping_order'} style={[stylesFont.FontFamilyText,
                            stylesFont.FontSize6, stylesMain.ItemCenterVertical, { color: '#111', width: width * 0.3, textAlign: 'center', }]}
                            >{'เตรียมจัดส่ง'}</Text> : <Text key={'shipping_order'} style={[stylesFont.FontFamilyText,
                            stylesFont.FontSize6, stylesMain.ItemCenterVertical, { color: '#111', width: width * 0.3, textAlign: 'center', }]}
                        ...
                        <TouchableOpacity key={'Review_order'} activeOpacity={1}
                            ...
                            <Text style={[stylesFont.FontFamilyText, stylesFont.FontSize6, stylesMain.ItemCenterVertical,
                            { color: '#20BDA1', width: width * 0.3, textAlign: 'center', }]}><IconFeather name='edit' size={15} />
                            เขียนรีวิว</Text>
                        </TouchableOpacity>,
                    ...
                    <View style={[stylesProfileTopic.Order_Box_priceText, { marginTop: 5, }]}>
                        {[
                            ...
                            <TouchableOpacity key={'return_order'} onPress={() => NavigationNavigateScreen({
                                goScreen: 'Return_products', setData: { selectedIndex: 1 }, navigation
                            })}>
                            ...
                            <TouchableOpacity key={'detail_order'} onPress={() => NavigationNavigateScreen({
